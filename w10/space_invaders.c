                ...
                Cannonball inv_balls[], int ib_count)
{
    ...
    for (int i = 0; i < ib_count; i++) {
        if (!inv_balls[i].active) continue;
    ...
    for (int i = 0; i < pb_count; i++) {
        if (!player_balls[i].active) continue;
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        if (rank == 0) fprintf(stderr, "Usage: mpirun -np N ./space_invaders_fixed rows cols\n");
        MPI_Finalize();
        return 1;
    }
    ...
    if (rows <= 0 || cols <= 0) {
        if (rank == 0) fprintf(stderr, "rows and cols must be positive\n");
        MPI_Finalize();
        return 1;
    }
    if (rows * cols != size - 1) {
        if (rank == 0) fprintf(stderr, "Error: rows*cols must equal number of invader processes (%d)\n", size - 1);
        MPI_Finalize();
        return 1;
    }
    if (rows > MAX_ROWS || cols > MAX_COLS) {
        if (rank == 0) fprintf(stderr, "Too large rows or cols (MAX_ROWS=%d MAX_COLS=%d)\n", MAX_ROWS, MAX_COLS);
        MPI_Finalize();
        return 1;
    }
    ...
    if (rank == 0) {
        ...
        while (running) {
            ...
            if (pb_count > 0 && pb_count > MAX_BALLS / 4) pb_count = compact_player_balls(player_balls, pb_count);
            ...
            if (ib_count > 0 && ib_count > MAX_BALLS / 4) ib_count = compact_inv_balls(inv_balls, ib_count);
            ...
            for (int src = 1; src < size; src++) {
                ...
                if (event > 0) {
                    ...
                    int inv_row = -1;
                    for (int r = rows - 1; r >= 0; r--) {
                        if (alive[r][col]) { inv_row = r; break; }
                    }
                    if (inv_row >= 0) {
            ...
            int target_row = -1;
            for (int r = rows - 1; r >= 0; r--) {
                if (alive[r][player_col]) { target_row = r; break; }
            }
            if (target_row >= 0) {
        ...
        for (int i = 0; i < msg_len; i++) term[i] = -1;
        ...
        if (!msg) { fprintf(stderr,"malloc failed invader\n"); MPI_Abort(MPI_COMM_WORLD,1); }

        while (1) {
            MPI_Bcast(msg, msg_len_local, MPI_INT, 0, MPI_COMM_WORLD);

            if (msg[0] == -1) break; // termination signaled
